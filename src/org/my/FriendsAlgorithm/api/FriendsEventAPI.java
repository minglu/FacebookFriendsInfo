...
public class FriendsEventAPI implements FaceBookAPI {
	...
	public String callAPI(String token, String id) {
		try {
			...
			URL url = new URL(
					"https://graph.facebook.com/fql?q=SELECT+name,+eid+FROM+event+WHERE+eid+IN(SELECT+eid+FROM+event_member+WHERE+uid+IN(SELECT+uid2+FROM+friend+WHERE+uid1=me())+OR+uid=me())&"
							+ token);
